...
satisfy the equation x\selectlanguage{ngerman}³ + 2x² + 10x -- 20 = 0. Leonardo showed by
...
\item
  ...
  Hint: Use \(Q^{5n + 5}\).
\item
  Let \selectlanguage{greek}φ \selectlanguage{english}be the positive root of x\textsuperscript{2} - x - 1 = 0. Prove
...
\item
  ...
  Admission is 50\selectlanguage{ngerman}¢, (denoted by H), and the box office starts with no
...
For n = 3, the matching of the 5 workable sequences with one of the 5
  ways of parenthesizing, given by the bijection (\selectlanguage{english}H, letter \selectlanguage{english}D is
  presented:

\end{enumerate}

H H H D D D \selectlanguage{english} (((abcd

H H D H D D \selectlanguage{english} ((a(bcd

H H D D H D \selectlanguage{english} ((ab(cd

H D H H D D \selectlanguage{english} (a((bcd

H D H D H D \selectlanguage{english}(a(b(cd
...
A bijection to the path problem is: an entry in the top row \selectlanguage{english} R. For
...
Parentheses \selectlanguage{english} H-D sequences

H-D \selectlanguage{english} Paths

Triangulations \selectlanguage{english} Parentheses

Insertions \selectlanguage{english} Paths

Trees \selectlanguage{english} Parentheses
...
The entries in the Pascal Triangle, \(

\begin{pmatrix}
n 
k 
\end{pmatrix}
\), satisfy the nice recursion \(
...
k - 1 
\end{pmatrix}
\) and also have a closed formula: \(
...
bins not allowing empty bins is \(
...
be accomplished? Among the m people there are \(
